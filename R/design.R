...
#
# The only epidemiological inputs are the generation interval g, the reporting
# delay pi_EY, and the exposures -- no latent/infectious-period distributions and
# no recovery rate.
#
...
# splines is a package dependency (see DESCRIPTION: Depends).
...
build_gi_design <- function(
    ...
    seed_rate      = 0       # pre-day-0 transmission seed: adds seed_rate * shifted survival of g
                            # to the renewal force (pre-history correction).
                            # Pairs with pre_hist_rate (the offset).  Usually 0.
) {
  ...
  if (!any(valid_mask)) {
    ...
    warning("build_gi_design: no points with Y > y_min_count; relaxed to Y > 0.")
  }
