...
#
# Note: lambda is chosen by K-fold cross-validation here. An earlier AIC/BIC
# grid-search route was removed in favour of CV.
# ==============================================================================
...
gi_select_lambda_cv <- function(
  ...
  if (!is.finite(cv_mean[best_i_min])) {
    warning("gi_select_lambda_cv: CV failed for all lambda values.")
    return(list(
      cv_mean = cv_mean,
      cv_sd = cv_sd,
      cv_se = cv_se,
  ...
  if (verbose)
    cat(sprintf("  Best lambda (CV): %.4e  (%s = %.4f)\n",
                lam_grid[best_i], error_measure, cv_mean[best_i]))
...
gi_select_lambda_cv_dow <- function(
  ...
  best_i_min <- which.min(cv_mean)
  if (!is.finite(cv_mean[best_i_min])) {
    warning("gi_select_lambda_cv_dow: CV failed for all lambda values.")
    return(list(
  ...
  if (verbose)
    cat(sprintf("  [DoW CV] Best lambda: %.4e  (%s = %.4f)\n",
                lam_grid[best_i], error_measure, cv_mean[best_i]))
