# ConvRt :: weekly.R
#
# Weekly-aggregated ConvRt: collapse the daily renewal design to weekly Poisson
# observations (Y_w ~ Poisson(sum-of-7-days mu)), then fit the same penalized
# smoothing-spline R_t.  Provides aggregate_design_weekly, the weekly retro/real-
# time wrappers, and a walk-forward gamma tuner, all built on the gi_* engine.
#
...
#   fit_convrt_weekly_retrospective(design, lam_grid, ...)   -> retro weekly fit
#   fit_convrt_weekly_realtime(design, lam_grid, gamma_grid, pi_E, ...) -> taper RT
...
# ------------------------------------------------------------------------------
fit_convrt_weekly_retrospective <- function(
  ...
  if (isTRUE(design$use_dow))
    stop("fit_convrt_weekly_retrospective: build the daily design with ",
...
# ------------------------------------------------------------------------------
# Real-time weekly fit (right-censored vintage).  Keep the partial right-edge week, tune
...
# ------------------------------------------------------------------------------
fit_convrt_weekly_realtime <- function(
  ...
  if (isTRUE(design$use_dow))
    stop("fit_convrt_weekly_realtime: build the daily design with dow_dates = NULL.")
  if (use_taper && is.null(pi_E))
    stop("fit_convrt_weekly_realtime: pi_E required when use_taper = TRUE.")
  if (is.null(lam) && is.null(lam_grid))
    stop("fit_convrt_weekly_realtime: provide either lam or lam_grid.")
  ...
  if (isTRUE(use_taper)) {
    ...
    if (is.null(gamma)) {
      if (is.null(gamma_grid))
        stop("fit_convrt_weekly_realtime: with use_taper and gamma = NULL, ",
             "provide gamma_grid.")
