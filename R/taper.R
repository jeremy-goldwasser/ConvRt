...
#
# fit_convrt_retrospective lives in wrappers.R.  A flu-specific wrapper
...
gi_build_tapered_penalty <- function(design, pi_E, quiet = FALSE) {
  # An optional gap-capping behaviour was removed; kept off here.
...
.gi_solve_taper <- function(
  ...
  # smoothing-spline penalty only (the ridge-penalty branch was removed)
  penalty <- "pspline"

  if (identical(design$basis, "natural") && tail == "constant") {
    stop("ConvRt: basis = 'natural' with tail = 'constant' is deprecated. ",
...
# ==============================================================================
# fit_convrt_retrospective lives in wrappers.R (smoothing-spline default,
...
# Forward-validation gamma tuner for convrt.  Takes the reporting-delay (E->Y)
...
gi_tune_gamma_fv <- function(
  ...
  # E0 bolus offset disabled (hardcoded to 0).
  E0_init <- 0
  ...
  if (verbose) cat("  Building designs for each holdout s...\n")
  designs    <- list()
  ...
  for (s in s_vals) {
    ...
    if (!is.null(fold_builder)) {
      fold <- tryCatch(
        fold_builder(s),
        error = function(e) {
          if (verbose) cat(sprintf("    s=%d fold_builder failed: %s\n", s, conditionMessage(e)))
          NULL
        }
      )
      ...
      # gamma is always tuned through a fold_builder: the per-fold design is
      # built from truncated exposures + the generation interval.
      stop("gi_tune_gamma_fv: a fold_builder must be supplied.")
    }
  }

  # ---- Compute D_{s+1} for each s (total exposure at s+1) -------------------
  # convrt Rt mode: I_vec_j IS the renewal force Lambda(t) = sum_k g_k X(t-k), so
  ...
  best_i <- best_i_min  # gamma is selected by the min-error rule
  best_gamma <- gamma_grid[best_i]
  if (verbose) cat(sprintf("  Best gamma: %.3e  (MAE = %.2f)\n", best_gamma, mae_vals[best_i]))
