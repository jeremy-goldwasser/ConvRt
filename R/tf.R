# ConvRt :: tf.R
#
# Trend-filter variant of convrt: instead of the natural-cubic-spline basis with
...
#                    Z_{t,s} = Lambda_s * pi_EY[t-s]              (convrt kernel)
#
# k = 4 -> cubic trend filter (R_t is piecewise cubic between data-driven knots).
# k = 2 -> piecewise linear.
#
# Experimental trend-filter variant: same convolution kernel as the spline path
# (build_gi_design already builds the right Xv = Lambda * pi_EY before spline
# aggregation), just swap the basis & penalty.
...
# The trend-filter variant needs CVXR (an optional / Suggested dependency).
# Checked at call time so the package still loads without CVXR installed.
.gi_require_cvxr <- function() {
  if (!requireNamespace("CVXR", quietly = TRUE))
    stop("The ConvRt trend-filter functions require the `CVXR` package. ",
         "Install it with install.packages(\"CVXR\").", call. = FALSE)
}
...
# ------------------------------------------------------------------------------
fit_convrt_tf_retrospective <- function(design, lam_grid, k_diff = 4L,
                                      nfold = 5L, seed = 1L,
                                      use_dow = FALSE, dow_anchor_lam = NULL,
                                      solver = "ECOS", verbose = FALSE) {
  if (!identical(design$basis, "tf_identity"))
    stop("fit_convrt_tf_retrospective: design must come from build_gi_design_tf().")
