# examples/demo.R
#
# Self-contained demo for ConvRt.  Simulate an outbreak with a knowingly
# non-trivial R_t trajectory (growth, peak, decline through 1, dip below 1,
# small rebound), recover R_t with ConvRt retrospectively and in real time, and
# plot the result.  No external data; depends only on the installed ConvRt
# package (remotes::install_github("jeremy-goldwasser/ConvRt")).

suppressPackageStartupMessages({ library(ConvRt); library(ggplot2); library(dplyr); library(tibble); library(patchwork) })

# Locate this script so the output PDF lands next to it, regardless of the
# working directory.
.this <- (function() {
...
combined <- (p_rt / p_obs) + plot_layout(heights = c(2, 1)) +
  plot_annotation(title = "ConvRt demo: two-wave R_t recovery from simulated cases",
                  theme = theme(plot.title = element_text(size = 17, face = "bold", hjust = 0.5)))

out_pdf <- file.path(convrt_root, "examples", "demo_rt.pdf")
